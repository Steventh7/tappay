...
var FormStore = function () {
  function FormStore(RootStore) {
    ...
    (0, _mobx.makeObservable)(this, {
      lock: _mobx.observable,
      currencyCode: _mobx.observable,
      tap: _mobx.observable,
      card: _mobx.observable
    });
  }

  _createClass(FormStore, [{
    key: "objectToQueryString",
    value: function objectToQueryString(obj, prefix) {
      var str = [],
          k,
          v;
      for (var p in obj) {
        ...
        v = obj[p];
        str.push((typeof v === "undefined" ? "undefined" : _typeof(v)) == "object" ? this.objectToQueryString(v, k) : k + "=" + encodeURIComponent(v));
      }
      return str.join("&");
    }
  }, {
    ...
    key: "generateForm",
    value: function generateForm(key) {
      var mid = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : null;
      ...
      self.elements = function (options) {
          ...
          function receiver(e) {
            ...
            if (iframe_obj && 0 === iframe_obj.src.indexOf(e.origin)) {
              ...
              if (e.data.layout) {
                ...
                self.RootStore.apiStore.getIP().then(function (ip) {
                  var iframeWin = document.getElementById("myFrame").contentWindow;
        ...
        self.elements.create = function (type, options_object, paymentOptions) {
          ...
          x.setAttribute("src", protocol + "//" + frameurl + "/tap_payment_widget_ui?" + self.objectToQueryString(options_object) + "&mid=" + mid + "&key=" + key + "&" + self.objectToQueryString(paymentOptions));
    ...
    key: "generateCardForm",
    value: function generateCardForm(id) {
      var self = this;

      var merchant_id = self.RootStore.configStore.config.gateway && self.RootStore.configStore.config.gateway.merchantId ? self.RootStore.configStore.config.gateway.merchantId : null;

      this.tap = this.generateForm(this.RootStore.configStore.gateway.publicKey, merchant_id);
          ...
          while (1) {
            switch (_context.prev = _context.next) {
              case 0:
                self = this;
                _context.next = 3;
                return self.tap.createToken(self.card).then(function (result) {
                  if (result.error) {
